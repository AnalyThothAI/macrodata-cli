...
class StooqProvider:
    ...
    quote_csv_url = "https://stooq.com/q/l/"

    def __init__(self, *, http_client: MacrodataHttpClient) -> None:
        self._http_client = http_client

    def get_latest(self, dataset: str) -> MacroObservation:
        text = self._get_quote_text(params={"s": dataset, "f": "sd2t2ohlcv", "h": "", "e": "csv"})
        rows = self._parse_quote_rows(dataset, text)
        if not rows:
        ...
        return self._parse_quote_row(dataset, rows[0])
    ...
    def _get_quote_text(self, *, params: dict[str, str]) -> str:
        try:
            with httpx.Client(timeout=self._http_client.timeout_sec) as client:
                response = client.get(self.quote_csv_url, params=params)
                response.raise_for_status()
        except httpx.TimeoutException as exc:
            raise MacrodataError(
                code="provider_timeout",
    ...
    def _parse_rows(self, dataset: str, text: str) -> list[dict[str, str]]:
        if self._is_access_instruction_response(text):
            raise MacrodataError(
                ...
                message=f"Stooq daily history requires a browser-issued API key for {dataset}",
                retryable=False,
                provider="stooq",
                exit_code=2,
            )
    ...
    def _parse_quote_rows(self, dataset: str, text: str) -> list[dict[str, str]]:
        if self._is_access_instruction_response(text):
            raise MacrodataError(
                code="provider_unavailable",
                message=f"Stooq quote endpoint returned access instructions for {dataset}",
                retryable=False,
                provider="stooq",
            )
        try:
            ...
            raise MacrodataError(
                code="provider_parse_error",
                message=f"Stooq quote CSV for {dataset} could not be parsed",
                retryable=False,
                provider="stooq",
            ) from exc
        if not rows:
        ...
        if not required_columns.issubset(rows[0]):
            raise MacrodataError(
                code="provider_parse_error",
                message=f"Stooq quote CSV for {dataset} is missing required columns",
                retryable=False,
                provider="stooq",
            )
        return rows
    ...
    def _parse_quote_row(self, dataset: str, row: dict[str, Any]) -> MacroObservation:
        observed_at = self._parse_observed_at(dataset, row.get("Date"))
        value = self._parse_close(dataset=dataset, observed_at=observed_at, raw_value=row.get("Close"))
        quote_time = "" if row.get("Time") is None else str(row.get("Time")).strip()
        source_ts = f"{observed_at}T{quote_time}" if quote_time else observed_at
        return MacroObservation(
            series_key=f"stooq:{dataset}",
            provider="stooq",
            dataset=dataset,
            ...
            latency_class="daily",
            data_quality="ok",
            provenance=[{"provider": "stooq", "source_url": self.quote_csv_url}],
        )
